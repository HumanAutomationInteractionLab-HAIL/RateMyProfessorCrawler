...
def LoadCachedPage(reviewerID,PageIndex):
    ...
    reviewfile = r"C:\Users\glass\Desktop\All review page cache\%s\reviewer-%s-%s-page.data"%(reviewerID,reviewerID,PageIndex)
    # the name of the file where we will store the object
    # Write to the file
...
def ReadReviewerProfile():
    ...
    wb = load_workbook(filename = 'amazon-reviewer-list-complete-profile -reviewer-2501-5000.xlsx')
    ...
    for rowIndex in range(1,nrows):
        ...
        try:
            rankIndexValue=sheet_ranges.cell(row = rowIndex, column = 1).value
            name=sheet_ranges.cell(row = rowIndex, column = 2).value
            reviewerID=sheet_ranges.cell(row = rowIndex, column = 3).value.strip()
            #print reviewerID
            TotalReviews=int(sheet_ranges.cell(row = rowIndex, column =4).value)
            #print TotalReviews
            HelpfulVotes=sheet_ranges.cell(row = rowIndex, column = 5).value
            crNumPercentHelpful=sheet_ranges.cell(row = rowIndex, column = 6).value
            crNumFanVoters=sheet_ranges.cell(row = rowIndex, column = 7).value
...
def ParseAllReviewInfo():
    ...
    for (rankIndexValue, 	name ,	reviewerID ,	TotalReviews ,	HelpfulVotes ,	crNumPercentHelpful ,	crNumFanVoters) in profileList:
        ...
        for indexPage in range(0,maxPages,crawlStep):
            ...
            try:
                productInfo=getInfo(rankIndexValue, 	name ,	reviewerID ,	TotalReviews ,	HelpfulVotes ,	crNumPercentHelpful ,	crNumFanVoters,indexPage)
                productReviewInfo.extend(productInfo)
            ...
            #break
        filename = r'C:\Users\glass\Desktop\Step 4. data analysis code\output folder s2500-5000\amazon-review-'+reviewerID+".csv"
        rootpath =r'C:\Users\glass\Desktop\Step 4. data analysis code\output folder s2500-5000'
        ensure_dir(rootpath)
